...
client.on('guildMemberAdd', member => {
   ...
   client.channels.get('700042323903381625').setName('⟫『 '+memberCount+' USER 』⟪');
  client.channels.get('700042348926861324').setName('⟫『 '+botCount+' BOT 』⟪');
...
client.on("message", message => {
 ...
 message.channel.send(`\`${message.guild.members.filter(m => m.presence.status !== 'all').size}\` **: anzahl der leute **`); 
 message.delete(); 
};     
});
...
client.on('message', function(message) {
    if (message.channel.type === "dm") {
        ...
        client.channels.get("700042810107101264").send({ embed: stewart });
    }
});
      ...
      if (message.author.bot) return//jackeo جاكيو
           message.delete();//jackeo جاكيو
             let args = message.content.split(' ').slice(1);//jackeo جاكيو
                 ...
                 if (virusname < 1) {//jackeo جاكيو//jackeo جاكيو
                     return message.channel.send("** tag someone ** ");//jackeo جاكيو
                                     }//jackeo جاكيو
           ...
           setTimeout(function() {
               ...
               m.edit({embed: new Discord.RichEmbed().setTitle(`** get into his acc ** `).setColor(0xFF0000)})
                ...
                setTimeout(function() {
               m.edit({embed: new Discord.RichEmbed().setTitle(`** save acc info ** `).setColor(0xFF0000)})
             }, 45500)//jackeo جاكيو
                setTimeout(function() {
               m.edit({embed: new Discord.RichEmbed().setTitle(`** upload ** `).setColor(0xFF0000)})
             }, 50500)//jackeo جاكيو
              setTimeout(function() {
